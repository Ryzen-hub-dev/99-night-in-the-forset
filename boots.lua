...
-- Hashing
local hasher = not isStudio and loadstring(game:HttpGet("https://sync-api.ryzenhub.menu/v1/lua/hasher"))()["hasher"] --or require(script.Parent.ModuleScript)['hasher']
...
local function display(userId, booster)
	...
	if overlayFrame then
		...
		if userInputService.TouchEnabled then
			overlayFrame.PlayerName.TextColor3 = booster and booster.color or Color3.fromRGB(0, 255, 150)
		else
			overlayFrame.PlayerName.PlayerName.TextColor3 = booster and booster.color or Color3.fromRGB(0, 255, 150)
		end
	end
end
